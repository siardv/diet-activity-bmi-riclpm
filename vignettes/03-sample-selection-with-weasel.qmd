---
title: "S3 · Sample selection with weasel"
---
...
**Attrition selectivity.** `weasel_selectivity()` compares retained and excluded respondents on covariates via standardised mean differences, the empirical check on whether completeness-based selection skews the sample that the missing-at-random discussion in the article leans on. The pipeline attaches age, gender, the three focal variables, and the SES stratum, so the released comparison speaks to selection on the study's own outcomes (excluded respondents are younger and slightly lighter; gender and activity are balanced):
...
sc <- data.frame(
  ...
  max_missing = 4L, n_gap_max = 5L, max_gap_len = 5L
)
...
Before committing to any rule, the scope pipeline maps the participation landscape. Since weasel 0.4.0 the scope shares the plan vocabulary (`max_gap_len`, `min_present`) and shows every respondent by default (`min_present = 1`); the call below pins the pre-0.4 exploration threshold of three observed waves explicitly:

```{r}
#| eval: false
weasel::set_weasel_scope(presence, "id", "t", max_gap_len = 1, min_present = 3)
